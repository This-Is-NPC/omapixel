...
| <kbd>g</kbd> | go to an exact X,Y pixel |
...
| <kbd>Ctrl</kbd>+<kbd>C</kbd> · <kbd>Ctrl</kbd>+<kbd>V</kbd> | copy selected pixels as JSON · paste a JSON colour matrix |
...
Press <kbd>g</kbd> to move farther without walking there. The panel starts in
the X field; type a zero-based column and press <kbd>Enter</kbd>, then type a
zero-based row and press <kbd>Enter</kbd> again. The cursor moves there and the
viewport follows it. Coordinates outside `0..width-1` and `0..height-1` are
refused without moving, and <kbd>Esc</kbd> cancels. **View → Go to pixel** opens
the same panel.

## Copying pixels as JSON

<kbd>Ctrl</kbd>+<kbd>C</kbd> copies the selected rectangle to the system
clipboard as a matrix of colours. Palette slot letters are deliberately absent:
the same letter can name different colours in two documents. A copied 3×2 area
looks like this:
...
Each array is one row, `#RRGGBB` is a drawn pixel, and `null` is transparency.
The same shape can be generated by a script and put on the clipboard without an
omapixel document around it.

<kbd>Ctrl</kbd>+<kbd>V</kbd> puts the matrix at the selection's top-left corner,
or at the keyboard cursor when there is no selection, or at `0,0` when neither
